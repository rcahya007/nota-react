...
const FormRincianBarang = ({ barang, setDataBarang }) => {
  ...
  useEffect(() => {
    if (barang.length > 0) {
      const total_harga = barang
        .map((harga) => harga.total_harga)
        .reduce((hargaSebelum, hargaSesudah) => hargaSebelum + hargaSesudah);
  ...
  return (
    <div className="clear-both mt-2">
      ...
      <div className="grid grid-cols-7  bg-slate-200 w-full border-y-2 border-slate-400 space-b">
        ...
        <div className="col-span-1 text-left py-2 px-2 font-bold">
          Deskripsi
        </div>
      ...
      {barang.length > 0 ? (
        <div>
          {barang.map((data) => (
            <div
              className="grid grid-cols-7 w-full border-b-2 border-slate-400 space-b align-middle"
              key={data.id}
            >
              <div className="col-span-2 py-2 px-2 flex items-center">
                {data.nama_barang}
              </div>
              ...
              <div className="col-span-1 text-center py-2 px-2">
                <button
                  className="border-2 border-slate-800 p-2 rounded-md hover:bg-slate-800 hover:duration-300 hover:text-white"
                  onClick={() => {
                    handleEdit(data.id);
                  }}
                >
      ...
      <div className="grid grid-cols-7 w-full border-b-2 border-slate-400 space-b">
        ...
        <div className="col-span-2 text-left py-2 px-2 font-bold">
          ...
          />
          <label htmlFor="Pemasukkan"> Pemasukkan</label>
          <br></br>
          <input
            type="radio"
            name="jenis_transaksi"
      ...
      <div className="flex items-center justify-center mt-5">
        <button
          className="bg-amber-400 py-2 px-6 rounded-md hover:bg-amber-500 border-2 border-white duration-300 hover:border-2 hover:border-black"
