...
const Nota = () => {
    ...
    const getAllTransactions = async () => {
        const fetch = await axios.get('http://benotareact.rendycahyae.my.id/transactions');
        setAllTransactions(fetch.data.DataBarang);
    }

    const getDetail = async (id) => {
        const getDetailNota = await axios.get('http://benotareact.rendycahyae.my.id/transactions/'+id);
    ...
    const getDelete = async (id) => {
        try {
            await axios.delete('http://benotareact.rendycahyae.my.id/transactions/'+id)
            getAllTransactions();
        } catch (error) {
            console.log(error)
        }
                ...
                <div className="container px-5 pt-7 mx-auto">
                    ...
                    <div className='float-right mb-5'>
                        <Link to={'/transaction/tambah'}>
                            <button className="bg-black text-white px-3 py-2 rounded-xl flex align-middle mr-2 font-bold text-lg items-center"><PlusIcon className="w-6 h-6 mr-2"/> Tambah Transaksi
                            </button>
                        </Link>
                    </div>
